...
static UPDATER_STATE: Mutex<Option<serde_json::Value>> = Mutex::new(None);

fn compare_versions(newer: &str, current: &str) -> bool {
    ...
    // Same numeric parts → the one with a pre-release suffix is older
    let a_is_pre = newer.contains('-');
    let b_is_pre = current.contains('-');
    if a_is_pre != b_is_pre {
        return b_is_pre; // current is pre-release, newer is stable
    }
    false
}
    ...
    tokio::task::spawn_blocking(move || {
        let ps_cmd = r#"
            try {
                ...
                $headers = @{ 'User-Agent' = 'PCCarePro-Updater' }
                $release = Invoke-RestMethod -Uri 'https://api.github.com/repos/thangdggr0004-cpu/PCCarePro/releases/latest' -Headers $headers -TimeoutSec 10
                $tag = $release.tag_name
                $body = $release.body
                $html_url = $release.html_url
                $download_url = ''
                if ($release.assets -and $release.assets.Count -gt 0) {
                    $portableExe = $release.assets | Where-Object { $_.name -match 'portable' -and $_.name -like '*.exe' } | Select-Object -First 1
                    if ($portableExe) {
                        $download_url = $portableExe.browser_download_url
                    } else {
        ...
        let output = std::process::Command::new("powershell")
            ...
            .creation_flags(exec::CREATE_NO_WINDOW)
            .output()
            .map_err(|e| e.to_string())?;
        ...
        if let Ok(v) = serde_json::from_str::<serde_json::Value>(&text) {
            if v["available"].as_bool() == Some(true) {
                ...
                if !tag.is_empty() && compare_versions(tag, current_ver) {
                    let info = serde_json::json!({
                        "hasUpdate": true,
                        "available": true,
                        "currentVersion": current_ver,
                        "latestVersion": tag,
                        "releaseNotes": v["body"].as_str().unwrap_or("Đã có bản cập nhật mới trên GitHub."),
                        "downloadUrl": v["download_url"].as_str().unwrap_or("https://github.com/thangdggr0004-cpu/PCCarePro/releases/latest"),
                        "releaseUrl": v["html_url"].as_str().unwrap_or("https://github.com/thangdggr0004-cpu/PCCarePro/releases")
                    });
                    if let Ok(mut st) = UPDATER_STATE.lock() {
                        *st = Some(serde_json::json!({
                    ...
                    let _ = app.emit("updater-event", serde_json::json!({
...
fn updater_download_exe(app: &tauri::AppHandle, info: &serde_json::Value) -> Result<String, String> {
    let url = info["downloadUrl"].as_str().unwrap_or("").to_string();
    if url.is_empty() {
        return Err("Không có URL tải xuống bản cập nhật.".to_string());
    }
    let tag = info["latestVersion"].as_str().unwrap_or("latest").to_string();

    // Portable: download next to the real exe. Dev: Desktop fallback.
    let current_exe = std::env::current_exe().map_err(|e| e.to_string())?;
    let target_dir = if cfg!(debug_assertions) {
        std::env::var("USERPROFILE").unwrap_or_else(|_| current_exe.parent().map(|p| p.to_string_lossy().to_string()).unwrap_or_default()) + "\\Desktop"
    } else {
        current_exe.parent().map(|p| p.to_string_lossy().to_string()).unwrap_or_default()
    };
    if target_dir.is_empty() {
        return Err("Không xác định được thư mục lưu file cập nhật.".to_string());
    }

    let mut file_name = format!("PCCareMasterPro_v{}.exe", tag);
    if let Some(pos) = url.rfind('/') {
        let base = &url[pos + 1..];
        if !base.is_empty() && base.to_lowercase().ends_with(".exe") {
            file_name = base.to_string();
        }
    }
    let dest_path = format!("{}//{}", target_dir, file_name);

    // Fetch total size via headers (curl.exe processes redirects)
    let total = {
        let out = std::process::Command::new("curl.exe")
            .args(["-sL", "-D", "-", "-o", "NUL", "--max-redirs", "5", &url])
            .creation_flags(exec::CREATE_NO_WINDOW)
            .output()
            .map_err(|e| e.to_string())?;
        ...
        text.lines()
            ...
            .find_map(|l| {
                ...
                if l.to_lowercase().starts_with("content-length:") {
                    l.split(':').nth(1).and_then(|s| s.trim().parse::<u64>().ok())
                } else {
                    None
                }
            })
    ...
    // Launch real download into a temp file so a retry never leaves a half exe
    let tmp_path = format!("{}.part", dest_path);
    let _ = std::fs::remove_file(&tmp_path);
    let mut child = std::process::Command::new("curl.exe")
        .args(["-sL", "-o", &tmp_path, "--max-redirs", "5", &url])
        .creation_flags(exec::CREATE_NO_WINDOW | exec::DETACHED_PROCESS)
        .spawn()
        .map_err(|e| e.to_string())?;

    // Poll file size to emit progress while curl streams to disk
    let start = std::time::Instant::now();
    loop {
        let size = std::fs::metadata(&tmp_path).map(|m| m.len()).unwrap_or(0);
        if total > 0 {
            let percent = ((size as f64 / total as f64) * 100.0).round() as u32;
            let _ = app.emit("updater-event", serde_json::json!({
        ...
        if start.elapsed().as_secs() > 300 {
            let _ = child.kill();
            let _ = std::fs::remove_file(&tmp_path);
            return Err("Tải xuống quá lâu (hơn 5 phút), đã hủy.".to_string());
        }
        std::thread::sleep(std::time::Duration::from_millis(300));
    }

    let final_size = std::fs::metadata(&tmp_path).map(|m| m.len()).unwrap_or(0);
    if final_size == 0 || (total > 0 && final_size < total) {
        let _ = std::fs::remove_file(&tmp_path);
        return Err("Tải file cập nhật thất bại.".to_string());
    }

    let _ = std::fs::remove_file(&dest_path);
    std::fs::rename(&tmp_path, &dest_path).map_err(|e| e.to_string())?;

    // Remember where the new exe lives so install-update can launch it
    if let Ok(mut st) = UPDATER_STATE.lock() {
        if let Some(s) = st.as_mut() {
            s["savePath"] = serde_json::json!(dest_path);
        }
    }
    Ok(dest_path)
}
...
#[tauri::command]
async fn download_update(app: tauri::AppHandle) -> Result<serde_json::Value, String> {
    tokio::task::spawn_blocking(move || {
        let info = UPDATER_STATE.lock()
            .ok()
            .and_then(|s| s.clone())
            .ok_or_else(|| "Chưa có bản cập nhật nào được phát hiện. Hãy kiểm tra cập nhật trước.".to_string())?;

        match updater_download_exe(&app, &info) {
            Ok(save_path) => {
                let _ = app.emit("updater-event", serde_json::json!({ "type": "update-downloaded" }));
                Ok(serde_json::json!({ "success": true, "downloadUrl": info["downloadUrl"], "savePath": save_path }))
            }
            Err(e) => {
                let _ = app.emit("updater-event", serde_json::json!({ "type": "error", "error": e }));
...
#[tauri::command]
fn install_update() -> Result<serde_json::Value, String> {
    let info = UPDATER_STATE.lock()
        .ok()
        .and_then(|s| s.clone())
        .ok_or_else(|| "Chưa tải file cập nhật nào.".to_string())?;
    let new_path = info["savePath"].as_str().unwrap_or("").to_string();
    if new_path.is_empty() || !std::path::Path::new(&new_path).exists() {
        return Err("File cập nhật chưa được tải xuống hoặc không tồn tại.".to_string());
    }

    // Dev mode: never delete the running binary
    if cfg!(debug_assertions) {
        let _ = exec::run_cmd_quiet("cmd", &["/c", "start", "cmd", "/c", "start", "", &new_path]);
        std::process::exit(0);
    }
    ...
    // VBS helper: wait for exit, retry deleting the old exe, then launch the new one
    let vbs_helper = format!(
        r#"Dim fso, shell
...
For i = 1 To 20
  ...
  If fso.FileExists("{cur}") Then
    fso.DeleteFile "{cur}", True
  End If
  On Error GoTo 0
  If Not fso.FileExists("{cur}") Then Exit For
  WScript.Sleep 1000
Next

On Error Resume Next
If fso.FileExists("{newp}") Then
  shell.Run """{newp}"""
End If
...
"#,
        cur = current,
        newp = new_path.replace('"', "\"\"")
    );

    let script_path = std::env::temp_dir().join("pccarepro_update_helper.vbs");
    std::fs::write(&script_path, vbs_helper.as_bytes()).map_err(|e| e.to_string())?;
